...
is($u->canonical, 'https://xyz/',
    'Default port 443 removed from canonical URI');
...
is($u->canonical, 'ftp://xyz',
    'FTP default port 21 removed from canonical URI');

$u = URI->new('http://xyz:08080');
$u->canonical;
is($u->as_string, 'http://xyz:08080',
    'canonical() does not mutate the original URI');

$u = URI->new('http://user@xyz:08080/');
is($u->canonical, 'http://user@xyz:8080/',
    'Leading zero stripped with userinfo present');

$u = URI->new('http://[::1]:08080/');
is($u->canonical, 'http://[::1]:8080/', 'Leading zero stripped with IPv6 host');

$u = URI->new('http://[::1]:080/');
is($u->canonical, 'http://[::1]/',
    'Default port with leading zeros removed for IPv6 host');
