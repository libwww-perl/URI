...
unless ($^O eq "MacOS") {
    chomp(
        my $pwd = (
            $^O =~ /mswin32/i ? `cd` : $^O eq 'VMS' ? `show default` : `pwd`
        )
    );
    if ($^O eq 'VMS') {
        $pwd =~ s#^\s+##;
    ...
    for (@INC) {
        ...
        next
            if $x =~ m|^/|
            or $^O =~ /os2|mswin32/i and $x =~ m#^(\w:[\\/]|[\\/]{2})#;
...
sub scheme_parse_test {
    ...
    my $tests = {
        ...
        'gopher://host' => {'gtype' => '1', 'as_string' => 'gopher://host',},
...
#		'host'          => 'ics.uci.edu',
        ...
        'news:*' => {'groupart' => '*', 'group' => '*', as_string => 'news:*'},
        ...
        'rlogin://aas@a.sn.no' => {'user' => 'aas', 'host' => 'a.sn.no'},
...
sub parts_test {
    ...
    is(
        $url->as_string,
        'http://web/1info?key%20words#this',
        ref($url) . '->as_string'
    );
    ...
    $url->path("howdy");
    is($url->as_string, 'http://web/howdy?a=%26#this',
        ref($url) . '->as_string');
...
#      unless $@;
    ...
    is(
        $url->as_string,
        'http://web?a&a=foo&%26%3D=%26%3D%2B',
        ref($url) . '->as_string'
    );

    my @a = $url->query_form;
    is(scalar(@a), 6, 'length');
    is_deeply(\@a, ['a', undef, 'a', 'foo', '&=', '&=+',], 'query_form',);
...
#      unless $@;
    ...
    ok($a[4] eq "" && $a[5] eq "b" && $a[10] eq "a" && $a[11] eq "b=c",
        'sequence',);
    ...
    is(
        $url->as_string,
        'http://web?a=foo&a=bar&b=foo&c=bar&c=foo',
        ref($url) . '->as_string'
    );
    ...
    $url->query_form(
        a => Foo::Bar::Array->new(['foo', 'bar']),
        b => 'foo',
        c => Foo::Bar::Array->new(['bar', 'foo'])
    );
    is(
        $url->as_string,
        'http://web?a=foo&a=bar&b=foo&c=bar&c=foo',
...
#    is($url->search '%09gisle', ref($url) . '->search');
    ...
    is($url->as_string, 'news:1234@a.sn.no',
        ref($url) . '->as_string: "<" and ">" are gone');
...
#    is($url->as_string, 'wais://host/foo/bar/wpath', ref($url) . '->as_string');
    ...
    is($crack, "http*UNDEF*UNDEF*host*80*/path*param*query*frag",
        'crack result');
...
#      $crack eq "ftp*UNDEF*UNDEF*UNDEF*21*foo/bar*UNDEF*UNDEF*UNDEF";
    ...
    $crack    = join("*", map { defined($_) ? $_ : "UNDEF" } @crack);
    note "Cracked result: $crack";
    is($crack, "ftp*anonymous*passwd*ftp.sn.no*21*/pub*UNDEF*UNDEF*UNDEF",
        'crack result');
...
#      $crack eq "mailto*aas*UNDEF*sn.no*UNDEF*aas\@sn.no*UNDEF*UNDEF*UNDEF";
    ...
    note "Cracked result: $crack";
    is($crack,
        "news*UNDEF*UNDEF*UNDEF*119*comp.lang.perl.misc*UNDEF*UNDEF*UNDEF",
        'crack result');
}
...
sub netloc_test {
    ...
    is(
        $url->as_string,
        'ftp://anonymous:pass@xn--hst-ula:12345',
        ref($url) . '->as_string'
    );
...
# legacy: this was legal before '[' and ']' were restricted to the host part of the URI (see: RFC 3513 & RFC 3986)
    ...
    is(
        $url->as_string,
        'file://h/%3B/%3F%3A%40%26%3D%23%25foo',
        ref($url) . '->as_string'
    );
    ...
    is(
        $url->as_string,
        'http://h/;%3B%2F%3F%3A%40&=%23%25?%3B%2F%3F%3A%40&=%23%25#;/?:@&=#%',
        ref($url) . '->as_string'
    );
    ...
    is(
        $url->as_string,
        'ftp://%3B%2F%3F%3A%40%26%3D%23%25@%3B%2F%3F%3A%40%26%3D%23%25/foo',
        ref($url) . '->as_string'
    );
...
sub newlocal_test {
    ...
    is(
        $url->as_string,
        URI::URL->new("file:$ss$dir/")->as_string,
        ref($url) . '->as_string'
    );
...
EOM
    ...
    push(
        @absolute_tests, ['x/y//../z' => 'http://a/b/c/x/y/z'],
        ['1'  => 'http://a/b/c/1'], ['0' => 'http://a/b/c/0'],
        ['/0' => 'http://a/0'],
...
#	 ["?foo",        "http://abc/a",   "http://abc/a?foo"],
        ...
        ["#foo", "http://abc/a", "http://abc/a#foo"],
        ["#foo", "file:a", "file:a#foo"], ["#foo", "file:/a", "file:/a#foo"],
    ...
    for (
        ...
        ["file:", "file:/etc/", "../"], ["file:foo", "file:/etc/", "../foo"],
        ...
        [
            ...
            "http://www.math.uio.no/doc/linux/",
            "../mail/top.html"
        ],
        )
    {
